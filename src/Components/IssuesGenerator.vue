<template>
  <div id="section-issue">
    <div class="feature-button">
      <button class="button issue-generate" @click="getIssues">
        Generate Issues
      </button>
      <button
        class="button issue-generate"
        v-bind:class="{ disable: isActive }"
    ...
    <textarea
      ...
      ref="inputField"
      v-model="clipData"
      v-bind:class="{ showArea: addDisplay }"
    >
    </textarea>
    <div class="align-bottom">
      ...
      <b-tag type="is-dark" size="is-medium" class="numberOfTickets">{{
        activeTicket
      }}</b-tag>
    </div>
    ...
    endedWorkOn(log) {
      ...
      histories = histories.filter(({ items }) => {
        return _.find(items, function (i) {
          return (
            i.field === "status" &&
            (i.toString === "Done" || i.toString === "Closed")
          );
        })
          ? true
          : false;
      ...
      return histories[histories.length - 1]
        ? histories[histories.length - 1].created
        : "No History Found";
    },
    ...
    async generateData() {
      ...
      for (let i = 0; i < this.modifiedIssues.length; i++) {
        const commitInfo = await this.getCommitInfo(
          this.modifiedIssues[i]["id"]
        );

        this.data.push({
          id: this.modifiedIssues[i]["key"],
          description: `${this.modifiedIssues[i]["key"]}: ${this.modifiedIssues[i]["fields"]["summary"]}`,
          comments: this.modifiedIssues[i].fields.comment.total,
          status: this.modifiedIssues[i].fields.status.name,
          pr: commitInfo.summary.pullrequest.overall.count
            ? commitInfo.summary.pullrequest.overall.state
            : "EMPTY",
          dateAssigned: this.modifiedIssues[i].dateStartedWorking,
          dateCompleted: this.modifiedIssues[i].dateEndedWorking,
        });

        // For copying data to clipboard
        this.clipData = this.clipData.concat(
          `${this.modifiedIssues[i]["key"]}: ${this.modifiedIssues[i]["fields"]["summary"]}\n`
        );
      }
